...
#define MAX_CONSECUTIVE 5  /* Maximum number of consecutive student from same class */
...
sem_t student_semaphore, professor_mutex, mod_mutex;
int consecutiveA, consecutiveB, professorReady;
...
static int initialize(student_info *si, char *filename) 
{
  ...
  sem_init(&student_semaphore, 0, MAX_SEATS); //blocking for seats
  sem_init(&professor_mutex, 0, 1);           //blocking for break
  sem_init(&mod_mutex, 0, 1);                 //blocking for modifying variables
...
void *professorthread(void *junk) 
{
  ...
  while (1) 
  {
    ...
    if (students_since_break == professor_LIMIT)
    {
      ...
      sem_wait(&professor_mutex);
      take_break();
      sem_post(&professor_mutex);
    }
