 ...
 *  
 *  getScore        ( int player )  take values 1 or 2 for players, returning players points  
 *  getCurrentTurn  ( none )        return either 1 or 2 for whos turn it is     
 *  getPieceCount   ( none )        return number of piece on board
 *  getGameBoard    ( none )        return current playing board
 *  isValidPlay     ( int column )  return if column is good to play
 *  playPiece       ( int column )  place piece of current player in column
 *  removePiece     ( int column )  remove piece at top of specified column
 *  printGameBoard  ( none )        print current playing board
 *  exitFunction    ( none )        quit
*/
...
public class GameBoard 
{
    ...
    try 
    {
      ...
      for(int i = 0; i < 6; i++) 
      {
        try 
        {
          ...
          //testing
          //uncomment the next line to see the whole line read in
          ...
          for( int j = 0; j < 7; j++ ) 
          {
            //testing- uncomment the next line to see each piece that was read in
  ...
  */
  ...
  {
    int player = this.getCurrentTurn();

    int winning = 0;

    //check 4 in a row
    winning += this.getScore(player) * 1000;

    //check 3 in a row with possible for 4 in a row
    ...
    for( int i = 0; i < 6; i++ ) 
    {
      for( int j = 0; j < 4; j++ ) 
      {
        if      ( ( this.playBoard[ i ][j] == player ) &&
                  ( this.playBoard[ i ][ j+1 ] == player ) &&
                  ( this.playBoard[ i ][ j+2 ] == player ) &&
                  ( this.playBoard[ i ][ j+3 ] == 0 )) winning += 100;

        else if ( ( this.playBoard[ i ][j] == player ) &&
                  ( this.playBoard[ i ][ j+1 ] == player ) &&
                  ( this.playBoard[ i ][ j+2 ] == 0 ) &&
                  ( this.playBoard[ i ][ j+3 ] == player )) winning += 100;
        ...
        // else if ( ( this.playBoard[ i ][j] == notplayer ) &&
        //           ( this.playBoard[ i ][ j+1 ] == notplayer ) &&
        //           ( this.playBoard[ i ][ j+2 ] == notplayer ) &&
        //           ( this.playBoard[ i ][ j+3 ] == 0 )) losing += 100;

        // else if ( ( this.playBoard[ i ][j] == notplayer ) &&
        //           ( this.playBoard[ i ][ j+1 ] == notplayer ) &&
        //           ( this.playBoard[ i ][ j+2 ] == 0 ) &&
        //           ( this.playBoard[ i ][ j+3 ] == notplayer )) losing += 100;
    ...
    for( int i = 0; i < 3; i++ ) 
    {
      for( int j = 0; j < 7; j++ ) 
      {
        if      ( ( this.playBoard[ i ][ j ] == 0 ) &&
                  ( this.playBoard[ i+1 ][ j ] == player ) &&
                  ( this.playBoard[ i+2 ][ j ] == player ) &&
                  ( this.playBoard[ i+3 ][ j ] == player ) ) winning += 100;

        // else if ( ( this.playBoard[ i ][ j ] == 0 ) &&
        //           ( this.playBoard[ i+1 ][ j ] == notplayer ) &&
        //           ( this.playBoard[ i+2 ][ j ] == notplayer ) &&
        //           ( this.playBoard[ i+3 ][ j ] == notplayer ) ) losing += 100;
      }
    }
    ...
    for( int i = 0; i < 3; i++ )
    {
      for( int j = 0; j < 4; j++ ) 
      {
        ...
        else if ( ( this.playBoard[ i+3 ][ j ] == player ) &&
                  ( this.playBoard[ i+2 ][ j+1 ] == player ) &&
        ...
        // else if ( ( this.playBoard[ i+3 ][ j ] == notplayer ) &&
        //           ( this.playBoard[ i+2 ][ j+1 ] == notplayer ) &&
        //           ( this.playBoard[ i+1 ][ j+2 ] == notplayer ) &&
        //           ( this.playBoard[ i ][ j+3 ] == 0 ) ) losing += 100;
    ...
    for( int i = 0; i < 3; i++ )
    {
      for( int j = 0; j < 4; j++ ) 
      {
        if      ( ( this.playBoard[ i ][ j ] == player ) &&
                  ( this.playBoard[ i+1 ][ j+1 ] == player ) &&
                  ( this.playBoard[ i+2 ][ j+2 ] == player ) &&
                  ( this.playBoard[ i+3 ][ j+3 ] == 0 ) ) winning += 100;

        else if ( ( this.playBoard[ i ][ j ] == player ) &&
                  ( this.playBoard[ i+1 ][ j+1 ] == player ) &&
                  ( this.playBoard[ i+2 ][ j+2 ] == 0 ) &&
                  ( this.playBoard[ i+3 ][ j+3 ] == player ) ) winning += 100;
        ...
        // else if ( ( ( this.playBoard[ i ][ j ] == notplayer ) &&
        //             ( this.playBoard[ i+1 ][ j+1 ] == notplayer ) &&
        //             ( this.playBoard[ i+2 ][ j+2 ] == notplayer ) &&
        //             ( this.playBoard[ i+3 ][ j+3 ] == 0 ) ) ) losing += 100;

        // else if   ( ( this.playBoard[ i ][ j ] == notplayer ) &&
        //             ( this.playBoard[ i+1 ][ j+1 ] == notplayer ) &&
        //             ( this.playBoard[ i+2 ][ j+2 ] == 0 ) &&
        //             ( this.playBoard[ i+3 ][ j+3 ] == notplayer ) ) losing += 100;
    ...
    for( int i = 0; i < 6; i++ ) 
    {
      for( int j = 0; j < 4; j++ ) 
      {
        if      ( ( this.playBoard[ i ][j] == player ) &&
                ( this.playBoard[ i ][ j+1 ] == player ) &&
                ( this.playBoard[ i ][ j+2 ] == 0 ) &&
                ( this.playBoard[ i ][ j+3 ] == 0 )) winning += 10;

        else if ( ( this.playBoard[ i ][j] == player ) &&
                ( this.playBoard[ i ][ j+1 ] == 0 ) &&
                ( this.playBoard[ i ][ j+2 ] == 0 ) &&
                ( this.playBoard[ i ][ j+3 ] == player )) winning += 10;
        ...
        //           ( this.playBoard[ i ][ j+1 ] == notplayer ) &&
        //           ( this.playBoard[ i ][ j+2 ] == 0 ) &&
        //           ( this.playBoard[ i ][ j+3 ] == 0 )) losing += 10;
  
        // else if ( ( this.playBoard[ i ][j] == notplayer ) &&
        //           ( this.playBoard[ i ][ j+1 ] == 0 ) &&
        //           ( this.playBoard[ i ][ j+2 ] == 0 ) &&
        //           ( this.playBoard[ i ][ j+3 ] == notplayer )) losing += 10;
    ...
    for( int i = 0; i < 4; i++ ) 
    {
      for( int j = 0; j < 7; j++ ) 
      {
        if      ( ( this.playBoard[ i ][ j ] == 0 ) &&
                  ( this.playBoard[ i+1 ][ j ] == player ) &&
                  ( this.playBoard[ i+2 ][ j ] == player ) ) winning += 10;

        // else if ( ( this.playBoard[ i ][ j ] == 0 ) &&
        //           ( this.playBoard[ i+1 ][ j ] == notplayer ) &&
        //           ( this.playBoard[ i+2 ][ j ] == notplayer ) ) losing += 10;
      }
    }
    ...
    for( int i = 0; i < 3; i++ )
    {
      for( int j = 0; j < 4; j++ ) 
      {
        if      ( ( this.playBoard[ i ][ j ] == player ) &&
                  ( this.playBoard[ i+1 ][ j+1 ] == player ) &&
                  ( this.playBoard[ i+2 ][ j+2 ] == 0 ) &&
                  ( this.playBoard[ i+3 ][ j+3 ] == 0 ) ) winning += 10;

        else if ( ( this.playBoard[ i ][ j ] == player ) &&
                  ( this.playBoard[ i+1 ][ j+1 ] == 0 ) &&
                  ( this.playBoard[ i+2 ][ j+2 ] == 0 ) &&
                  ( this.playBoard[ i+3 ][ j+3 ] == player ) ) winning += 10;
        ...
        //             ( this.playBoard[ i+1 ][ j+1 ] == notplayer ) &&
        //             ( this.playBoard[ i+2 ][ j+2 ] == 0 ) &&
        //             ( this.playBoard[ i+3 ][ j+3 ] == 0 ) ) ) losing += 10;

        // else if   ( ( this.playBoard[ i ][ j ] == notplayer ) &&
        //             ( this.playBoard[ i+1 ][ j+1 ] == 0 ) &&
        //             ( this.playBoard[ i+2 ][ j+2 ] == 0 ) &&
        //             ( this.playBoard[ i+3 ][ j+3 ] == notplayer ) ) losing += 10;
    ...
    //check 1 with empty spaces?
    //TODO: not sure if need one space checking?

    //return winning - losing;
    return winning;
  }

  public int terminalEvaluation()
  {
    int player = this.getCurrentTurn();
    int notplayer = (player == 1) ? 2 : 1;
    return this.getScore(player) - this.getScore(notplayer);
  }
  ...
  */
  ...
  public void removePiece( int column ) 
  {
    ...
    //testing
    //WARNING: uncommenting the next 2 lines will potentially
